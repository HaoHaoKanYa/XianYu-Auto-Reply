...
class XianyuLive:
    ...
    def __init__(self, cookies_str=None, cookie_id: str = "default", user_id: int = None):
        ...
        # 消息去重缓存 - 防止重复处理同一条消息
        self.processed_messages = {}  # 格式: {message_id: timestamp}
        self.message_cache_ttl = 60  # 消息缓存有效期：60秒
        
        # 回复去重缓存 - 防止短时间内重复发送相同回复
        self.sent_replies = {}  # 格式: {(chat_id, user_id, reply_content): timestamp}
        self.reply_cache_ttl = 5  # 回复缓存有效期：5秒（同一用户5秒内不重复发送相同内容）
        self.reply_lock = asyncio.Lock()  # 回复锁，防止并发发送
    ...
    async def handle_message(self, message_data, websocket):
        ...
        try:
            ...
            # 【消息去重】使用消息的JSON字符串作为唯一标识（简单可靠）
            try:
                # 清理过期的消息缓存
                current_time = time.time()
                expired_keys = [k for k, v in self.processed_messages.items() if current_time - v > self.message_cache_ttl]
                for k in expired_keys:
                    del self.processed_messages[k]
                
                # 使用消息的JSON字符串的hash作为ID（排除时间戳字段）
                import hashlib
                ...
                if message_hash in self.processed_messages:
                    logger.info(f"【{self.cookie_id}】⏭️ 跳过重复消息 (hash: {message_hash[:8]}...)")
                    return
                
                # 标记为已处理
                self.processed_messages[message_hash] = current_time
                logger.debug(f"【{self.cookie_id}】✅ 新消息已标记 (hash: {message_hash[:8]}...)")
            except Exception as e:
                logger.warning(f"【{self.cookie_id}】消息去重处理异常: {self._safe_str(e)}，继续处理消息")
            ...
            if reply:
                ...
                async with self.reply_lock:
                    try:
                        # 清理过期的回复缓存
                        current_time = time.time()
                        expired_keys = [k for k, v in self.sent_replies.items() if current_time - v > self.reply_cache_ttl]
                        for k in expired_keys:
                            del self.sent_replies[k]
                        
                        # 生成回复标识（chat_id + user_id + 回复内容前100字符）
                        reply_key = (chat_id, send_user_id, reply[:100] if len(reply) > 100 else reply)
                        
                        # 检查是否刚刚发送过相同回复
                        if reply_key in self.sent_replies:
                            time_diff = current_time - self.sent_replies[reply_key]
                            logger.info(f"【{self.cookie_id}】⏭️ 跳过重复回复: {send_user_name} (距上次 {time_diff:.2f}秒)")
                            return
                        
                        # 标记为已发送
                        self.sent_replies[reply_key] = current_time
                    except Exception as e:
                        logger.warning(f"【{self.cookie_id}】回复去重处理异常: {self._safe_str(e)}，继续发送")
