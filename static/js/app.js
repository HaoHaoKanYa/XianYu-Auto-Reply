...
// 登出功能
async function logout(clearRememberMe = false) {
    try {
        if (authToken) {
            await fetch('/logout', {
                method: 'POST',
...
async function loadReminderPage() {
    try {
        ...
        console.error('加载提醒收货页面失败:', error);
...
function onReminderCookieChange() {
    const select = document.getElementById('reminderCookieFilter');
    if (select) {
        reminderCookieId = select.value;
        console.log('选择的提醒账号:', reminderCookieId);
    }
}
...
async function openReminderSettings() {
    ...
    try {
        // 加载提醒设置
        const response = await fetch(`/reminder-settings/${reminderCookieId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        });
        ...
        document.getElementById('firstDelayValue').value = settings.first_delay_value;
        document.getElementById('firstDelayUnit').value = settings.first_delay_unit;
        document.getElementById('reminderInterval').value = settings.reminder_interval;
        document.getElementById('maxReminderCount').value = settings.max_reminder_count;
        document.getElementById('excludeBlacklist').checked = settings.exclude_blacklist;
        document.getElementById('excludeDispute').checked = settings.exclude_dispute;
        document.getElementById('excludeCompetitor').checked = settings.exclude_competitor;

        // 显示模态框
        const modal = new bootstrap.Modal(document.getElementById('reminderSettingsModal'));
        modal.show();
...
async function saveReminderSettings() {
    ...
    try {
        const settings = {
            ...
            first_delay_value: parseInt(document.getElementById('firstDelayValue').value),
            first_delay_unit: document.getElementById('firstDelayUnit').value,
            reminder_interval: parseInt(document.getElementById('reminderInterval').value),
            max_reminder_count: parseInt(document.getElementById('maxReminderCount').value),
            exclude_blacklist: document.getElementById('excludeBlacklist').checked,
            exclude_dispute: document.getElementById('excludeDispute').checked,
            exclude_competitor: document.getElementById('excludeCompetitor').checked
        };

        // 验证输入
        if (settings.first_delay_value < 1 || settings.first_delay_value > 30) {
            showToast('首次延迟时间必须在1-30之间', 'warning');
            return;
        }

        if (settings.reminder_interval < 1 || settings.reminder_interval > 10) {
            showToast('提醒间隔必须在1-10天之间', 'warning');
            return;
        }

        if (settings.max_reminder_count < 1 || settings.max_reminder_count > 10) {
            showToast('最大提醒次数必须在1-10之间', 'warning');
            return;
        }
        ...
        showToast('保存提醒设置失败', 'error');
    }
}

// 打开提醒记录查看器
async function openReminderRecords() {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }
    ...
    try {
        // 加载提醒记录
        const response = await fetch(`/reminder-records/${reminderCookieId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        });
        ...
        // 渲染提醒记录表格
        const tbody = document.getElementById('reminderRecordsTableBody');
        tbody.innerHTML = '';

        if (records.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" class="text-center">暂无提醒记录</td></tr>';
        } else {
            const now = new Date();
            records.forEach(record => {
                ...
                if (record.next_reminder_time) {
                    const nextTime = new Date(record.next_reminder_time);
                    const isPast = nextTime <= now;
                    const timeClass = isPast ? 'text-danger fw-bold' : 'text-muted';
                ...
                row.innerHTML = `
                    ...
                    <td>${record.reminder_count}</td>
                    <td>${record.last_reminder_time || '-'}</td>
                    <td>${nextTimeDisplay}</td>
        ...
        // 显示模态框
        const modal = new bootstrap.Modal(document.getElementById('reminderRecordsModal'));
        modal.show();

    } catch (error) {
        console.error('加载提醒记录失败:', error);
        showToast('加载提醒记录失败', 'error');
    }
}
...
// 打开黑名单管理器
async function openBlacklistManager() {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }
    ...
    try {
        // 加载黑名单用户
        const response = await fetch(`/blacklist-users/${reminderCookieId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        });
        ...
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center">暂无黑名单用户</td></tr>';
        } else {
            users.forEach(user => {
                const row = document.createElement('tr');
                row.innerHTML = `
                    ...
                    <td>${user.reason || '-'}</td>
                    <td>
                        <button class="btn btn-sm btn-danger" onclick="removeFromBlacklist('${user.buyer_id}')">
...
async function addToBlacklist() {
    ...
    const buyerId = document.getElementById('blacklistBuyerId').value.trim();
    const reason = document.getElementById('blacklistReason').value.trim();

    if (!buyerId) {
        showToast('请输入买家ID', 'warning');
        return;
    }
    ...
    try {
        const response = await fetch(`/blacklist-users/${reminderCookieId}`, {
            ...
            body: JSON.stringify({ buyer_id: buyerId, reason: reason })
        });
        ...
        if (data.success) {
            ...
            // 清空表单
            document.getElementById('blacklistBuyerId').value = '';
        ...
        showToast('添加黑名单失败', 'error');
    }
}

// 从黑名单移除
async function removeFromBlacklist(buyerId) {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }

    if (!confirm(`确定要将买家 ${buyerId} 从黑名单中移除吗？`)) {
        return;
    }

    try {
        const response = await fetch(`/blacklist-users/${reminderCookieId}/${buyerId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        ...
        showToast('移除黑名单失败', 'error');
    }
}

// 打开同行管理器
async function openCompetitorManager() {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }
    ...
    try {
        // 加载同行用户
        const response = await fetch(`/competitor-users/${reminderCookieId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        });
        ...
        if (users.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" class="text-center">暂无同行用户</td></tr>';
        } else {
            users.forEach(user => {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${user.buyer_id}</td>
                    <td>${user.detected_reason || '-'}</td>
                    <td>
                        <button class="btn btn-sm btn-danger" onclick="removeFromCompetitor('${user.buyer_id}')">
                            <i class="bi bi-trash"></i> 移除
                        </button>
                    </td>
                `;
        ...
        console.error('加载同行用户失败:', error);
        showToast('加载同行用户失败', 'error');
    }
}

// 添加同行用户
async function addToCompetitor() {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }

    const buyerId = document.getElementById('competitorBuyerId').value.trim();
    const reason = document.getElementById('competitorReason').value.trim();
    ...
    try {
        const response = await fetch(`/competitor-users/${reminderCookieId}`, {
            ...
            body: JSON.stringify({ buyer_id: buyerId, detected_reason: reason })
        });

        if (!response.ok) {
            throw new Error('添加同行用户失败');
        }
        ...
        if (data.success) {
            showToast('添加成功', 'success');
            // 清空表单
            document.getElementById('competitorBuyerId').value = '';
            document.getElementById('competitorReason').value = '';
            // 重新加载同行用户
            openCompetitorManager();
        } else {
            showToast('添加失败', 'error');
        }

    } catch (error) {
        console.error('添加同行用户失败:', error);
        showToast('添加同行用户失败', 'error');
    }
}

// 从同行用户移除
async function removeFromCompetitor(buyerId) {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }

    if (!confirm(`确定要将买家 ${buyerId} 从同行用户中移除吗？`)) {
        return;
    }

    try {
        const response = await fetch(`/competitor-users/${reminderCookieId}/${buyerId}`, {
            method: 'DELETE',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        ...
        console.error('移除同行用户失败:', error);
        showToast('移除同行用户失败', 'error');
    }
}

// 查看提醒统计
async function viewReminderStatistics() {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }
    ...
    try {
        const response = await fetch(`/reminder-statistics/${reminderCookieId}`, {
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
        });
        ...
        document.getElementById('statCancelledOrders').textContent = stats.cancelled_orders;
        document.getElementById('statTotalReminders').textContent = stats.total_reminders;

        // 显示模态框
        const modal = new bootstrap.Modal(document.getElementById('reminderStatisticsModal'));
        modal.show();
...
// 扫描已发货订单
async function scanShippedOrders() {
    if (!reminderCookieId) {
        showToast('请先选择账号', 'warning');
        return;
    }

    if (!confirm('确定要扫描该账号的已发货订单并创建提醒记录吗？\n\n这将为所有未创建提醒记录的已发货订单创建提醒记录。')) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/reminder-scan-orders/${reminderCookieId}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
...
async function testSendReminder() {
    ...
    if (!confirm('确定要发送一条测试提醒消息吗？\n\n这将向第一个待提醒订单的买家发送提醒消息。')) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/reminder-test-send/${reminderCookieId}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
...
async function triggerRemindersNow() {
    ...
    if (!confirm('确定要立即触发所有待提醒订单吗？\n\n这将把所有待提醒订单的下次提醒时间设置为当前时间，提醒任务将在1分钟内执行。\n\n⚠️ 此功能主要用于测试，请谨慎使用！')) {
        return;
    }
    ...
    try {
        ...
        const response = await fetch(`/reminder-trigger-now/${reminderCookieId}`, {
            method: 'POST',
            headers: {
                'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
            }
